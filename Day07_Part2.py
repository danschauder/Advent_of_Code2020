    ...
    if children != 'no other bags.':
        ...
        for child in children_split:
            ...
            if parent not in bags_graph.keys():
                bags_graph[parent]={parsed_child[2]:int(parsed_child[1])}
            else:
                bags_graph[parent].update({parsed_child[2]:int(parsed_child[1])})
    else:
        pass


def count_children(target_bag_name):
    print(target_bag_name)
    if target_bag_name not in bags_graph.keys():
        return 0
    else:
        children_count=0
        for child in bags_graph[target_bag_name].keys():
            for i in range(bags_graph[target_bag_name][child]):
                children_count+=count_children(child)
            children_count+=bags_graph[target_bag_name][child]
