...
for p in passports:
    ...
    reg2=re.compile(r"""
                    ...
                    [\s]{0,2}
                    """,re.VERBOSE)
    parsed = [b for b in re.split(reg2,p) if len(b)>0]
    form = defaultdict()
    for i in range(0,len(parsed)-1,2):
        form[parsed[i]]=parsed[i+1]
    good=True
    for field in fields:
        if field not in form.keys():
            ...
            if field=='byr':
                if not (int(form[field])>=1920 and int(form[field])<=2002 and len(form[field])==4):
                    good=False
            elif field=='iyr':
                if not (int(form[field])>=2010 and int(form[field])<=2020 and len(form[field])==4):
                    good=False
            elif field=='eyr':
                if not (int(form[field])>=2020 and int(form[field])<=2030 and len(form[field])==4):
                    good=False
            elif field=='hgt':
                hgt_reg = re.compile(r"""
                                    (\d*)([a-z]{2,2})$
                                    """,re.VERBOSE)
                height_split=[h for h in re.split(hgt_reg,form[field]) if len(h)>0]
                if not (len(height_split)==2 and height_split[1] in ('in','cm')):
                    good=False
                elif height_split[1]=='cm':
                    if not (int(height_split[0])>=150 and int(height_split[0])<=193):
                        good=False
                else:
                    if not (int(height_split[0])>=59 and int(height_split[0])<=76):
                        good=False
            elif field=='hcl':
                hcl_reg = re.compile(r"""
                                    ^\#[\w]{6,6}$
                                    """,re.VERBOSE)
                hcl_split = re.findall(hcl_reg,form[field])
                if len(hcl_split)==0:
                    good=False
            elif field=='ecl':
                if form[field] not in ('amb','blu','brn','gry','grn','hzl','oth'):
                    good=False
            elif field=='pid':
                pid_reg = re.compile(r"""
                                    ^[\d]{9,9}$
                                    """,re.VERBOSE)
                pid_split = re.findall(pid_reg,form[field])
                if len(pid_split)==0:
                    good=False
    if good:
        valid+=1
