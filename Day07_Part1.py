...
input = """mirrored silver bags contain 4 wavy gray bags.
...
reg1 = re.compile(r"\n",re.VERBOSE)
rules = re.split(reg1,input)
bags_graph = defaultdict()
for rule in rules:
    reg2 = re.compile(r"""^
                    (.*)
                    \sbags\scontain\s
                    ...
                    $
                    """,re.VERBOSE)
    parsed_rule=re.split(reg2,rule)
    parent = parsed_rule[1]
    children = parsed_rule[2]
    if children != 'no other bags.':
        ...
        for child in children_split:
            parsed_child = re.split(reg3,child)
            if parsed_child[1] in bags_graph.keys():
                bags_graph[parsed_child[1]]['parents'].append(parent)
            else:
                bags_graph[parsed_child[1]]={'visited':False, 'parents':[parent]}
    else:
        pass


def count_ancestors(target_bag_name):
    print(f"Target node is {target_bag_name}")
    if target_bag_name not in bags_graph.keys():
        return 0
    else:
        parent_count=0
        for parent in bags_graph[target_bag_name]['parents']:
            if parent not in bags_graph.keys():
                parent_count+=1
                bags_graph[parent]={'visited':True}
            elif not bags_graph[parent]['visited']:
                bags_graph[parent]['visited']=True
                parent_count+=1 + count_ancestors(parent)
        return parent_count

    

print(count_ancestors('shiny gold'))