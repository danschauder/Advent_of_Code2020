...
groups = re.split(reg,input)


def inner_join(results,spaces):
    if len(spaces)==1:
        return results.intersection(spaces[0])
    else:
        return inner_join(results.intersection(spaces[0]),spaces[1:])

ans=0
for group in groups:
    # print(group)
    ##strip whitespace
    reg2=re.compile(r'\s+',re.VERBOSE)
    people = [set(a) for a in re.split(reg2,group)]
    if len(people)==1:
        intersection=people[0]
    else:
        intersection = inner_join(people[0],people[1:])
    ans+=len(intersection)

    # print(processed_group)
    # group_set = set(processed_group)
    # ans+=len(group_set)
print(ans)